...
export function GridEditor({ initialCells, blockedCells, color, onCancel, onSave }: { initialCells: number[]; blockedCells: number[]; color?: string; onCancel: () => void; onSave: (cells: number[]) => string | null }) {
  ...
  return <View style={{ gap: 12 }}><PanGestureHandler minDist={4} onGestureEvent={handleGestureEvent} onHandlerStateChange={handleStateChange}><View testID="grid-editor" onLayout={event => setWidth(event.nativeEvent.layout.width)} style={{ width: '100%', aspectRatio: 1, backgroundColor: '#FFF', overflow: 'hidden' }}>
    {width > 0 && Array.from({ length: 64 }, (_, cell) => { const selected = cells.includes(cell); const unavailable = blocked.has(cell); const selectedColor = normalizeModuleColor(color); return <Pressable key={cell} accessibilityRole="button" accessibilityLabel={`编辑第 ${cell + 1} 格`} disabled={unavailable} onPress={() => toggleCell(cell)} style={{ position: 'absolute', left: (cell % 8) * width / 8, top: Math.floor(cell / 8) * width / 8, width: width / 8, height: width / 8, borderWidth: selected ? 1.5 : .5, borderColor: selected ? colors.ink : colors.line, backgroundColor: unavailable ? '#E4E9E6' : selected ? selectedColor : '#FAFCFB', justifyContent: 'center', alignItems: 'center' }}><Text style={{ fontSize: 9, color: unavailable ? colors.muted : selected ? moduleColorTextColor(selectedColor) : colors.muted }}>{selected ? '✓' : ''}</Text></Pressable>; })}
