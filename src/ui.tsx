...
// Visual breathing room layered on top of the native SafeAreaView in App.tsx.
// SafeAreaView supplies the device-specific inset on iOS; these values keep the
// Web preview and Android layouts equally comfortable.
...
export function Chip({ label, selected, onPress, tone }: { label: string; selected: boolean; onPress: () => void; tone?: LocationTone }) {
  ...
  return <Pressable accessibilityRole="radio" accessibilityState={{ checked: selected }} accessibilityLabel={label} onPress={onPress} style={[s.chip, selected && s.chipSelected, palette && { borderColor: selected ? palette.border : colors.line, backgroundColor: selected ? palette.background : colors.surface }]}><Text style={{ color: selected ? palette?.text ?? colors.accent : colors.ink }}>{label}</Text></Pressable>;
}
...
export const s = StyleSheet.create({
  ...
  wardrobeGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 12 }, wardrobeCard: { width: '48%', minWidth: 140, padding: 12, borderRadius: 18, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.line, gap: 8 }, wardrobeImage: { width: '100%', height: 150, borderRadius: 14, backgroundColor: colors.gridEmpty }, wardrobePlaceholder: { width: '100%', height: 150, borderRadius: 14, backgroundColor: colors.gridEmpty, alignItems: 'center', justifyContent: 'center', gap: 6 },
