...
import { HomePage, ItemRows, LayoutPage, RoomsPage, WardrobePage } from './src/pages';
...
type Tab = 'home' | 'rooms' | 'wardrobe' | 'items' | 'settings';
...
export default function App() {
  ...
  const saveName = () => {
    ...
    if (editor.kind === 'category' && editor.id && isSystemCategory(editor.id)) return setError('系统分类不能修改');
  ...
  const saveItem = (draft: Omit<Item, 'id'>, editingId?: string) => {
    ...
    if (previous?.categoryId === 'clothes' && draft.categoryId !== 'clothes') return '衣物分类不能修改';
  ...
  const deleteHome = (homeToDelete: Home) => {
    ...
    const roomIds = new Set(rooms.filter(r => r.homeId === homeToDelete.id).map(r => r.id));
    setConfirmation({ title: `删除家“${homeToDelete.name}”？`, message: `将删除 ${roomIds.size} 个房间、${containers.filter(c => roomIds.has(c.roomId)).length} 个模块和 ${items.filter(i => roomIds.has(i.roomId)).length} 件物品，删除后无法恢复。`, run: () => {
      const result = removeHomeContents({ homes, rooms, containers, items }, homeToDelete.id, true);
      if ('error' in result) return;
      setData(d => ({ ...d, homes: result.homes, rooms: result.rooms, containers: result.containers, items: result.items }));
  ...
  const deleteCategory = (category: Category) => {
    if (isSystemCategory(category.id)) return;
  ...
  const nav: [Tab, IconName, string][] = [['home', 'home', '首页'], ['rooms', 'grid', '房间'], ['wardrobe', 'shopping-bag', '衣柜'], ['items', 'package', '物品'], ['settings', 'settings', '设置']];
  ...
  return <GestureHandlerRootView style={{ flex: 1 }}><SafeAreaView style={s.screen}><StatusBar barStyle="dark-content" /><View style={s.content}>
    ...
    {tab === 'wardrobe' && <WardrobePage items={currentItems} onOpenItem={item => setSelectedItemId(item.id)} />}
